...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, n_in, n_out, n_layers):
        ...
        self.res_block = nn.Sequential(
                                        nn.ReLU(), nn.Conv2d(n_in, n_hid, 3, padding='same'),
...
class Encoder(nn.Module):
    ...
    def __init__(self, inp_ch, n_hid, block_per_group, emb_dim, group_count=4):
        ...
        self.group1 = nn.Sequential(OrderedDict([(f'G1_{i+1}', EncoderBlock(n_hid, n_hid, n_layers)) for i in range(block_per_group)]))
        self.group2 = nn.Sequential(OrderedDict([(f'G2_{i+1}', EncoderBlock(1*n_hid if i==0 else 2*n_hid, 2*n_hid, n_layers)) for i in range(block_per_group)]))
        self.group3 = nn.Sequential(OrderedDict([(f'G3_{i+1}', EncoderBlock(2*n_hid if i==0 else 4*n_hid, 4*n_hid, n_layers)) for i in range(block_per_group)]))
        self.group4 = nn.Sequential(OrderedDict([(f'G4_{i+1}', EncoderBlock(4*n_hid if i==0 else 8*n_hid, 8*n_hid, n_layers)) for i in range(block_per_group)]))
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, n_in, n_out, n_layers):
        ...
        self.identity_conv = nn.Conv2d(n_in, n_out, 1) if n_in != n_out else nn.Identity()
        self.res_block = nn.Sequential(
                                        nn.ReLU(), nn.Conv2d(n_in, n_hid, 1),
                                        nn.ReLU(), nn.Conv2d(n_hid, n_hid, 3, padding='same'),
                                        nn.ReLU(), nn.Conv2d(n_hid, n_hid, 3, padding='same'),
                                        nn.ReLU(), nn.Conv2d(n_hid, n_out, 3, padding='same')
                                        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, n_init, n_hid, block_per_group, emb_dim, out_ch, group_count=4):
        ...
        self.group1 = nn.Sequential(OrderedDict([(f'G1_{i+1}', EncoderBlock(n_init if i==0 else 8*n_hid, 8*n_hid, n_layers)) for i in range(block_per_group)]))
        self.group2 = nn.Sequential(OrderedDict([(f'G2_{i+1}', EncoderBlock(8*n_hid if i==0 else 4*n_hid, 4*n_hid, n_layers)) for i in range(block_per_group)]))
        self.group3 = nn.Sequential(OrderedDict([(f'G3_{i+1}', EncoderBlock(4*n_hid if i==0 else 2*n_hid, 2*n_hid, n_layers)) for i in range(block_per_group)]))
        self.group4 = nn.Sequential(OrderedDict([(f'G4_{i+1}', EncoderBlock(2*n_hid if i==0 else 1*n_hid, 1*n_hid, n_layers)) for i in range(block_per_group)]))
